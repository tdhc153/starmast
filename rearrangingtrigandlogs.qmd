...
Another type of trigonometric trick you are likely to come across are known as R equations. These allow you a way to rewrite equations in terms of sin and cos in to an equation in either sin or cos. They make use of the addition formula.
...
R equations will have you rewrite an equation in the form $A\cos(x)\pmB\sin(x)$ as $R\cos(x\pm\theta)$ or $R\sin(x\pm\theta)$. Let's try an example!

::: {.callout-note appearance="simple"}
## Example 5

$$2\sin(x)+6\cos(x)= R\cos(x+\theta)$$
Determine R and $\theta$.

You will start by rewriting the right hand side of the equation using the appropriate addition formulae. This gives $2\sin(x)+6\cos(x)=R\cos(\theta)\cos(x)-R\sin(\theta)\sin(x)$. Now, we can compare coefficients on either side of the equation. For this to be true for all values of x, the coefficients before sin and cos must match on each side. This means that $2=R\cos(\theta)$ and $6=R\sin(\theta)$. 
...
Therefore, you can say $2\sin(x)+6\cos(x)= 2\sqrt{10}\cos(x+1.25)$. This method can be helpful if given an equation such as $2\sin(x)+6\cos(x)= 5$, where you would need to rewrite it as $2\sqrt{10}\cos(x+1.25)=5$ to solve. 
...
Thus, refer to our example 8 to see that $3x=2x+1$. Subtracting 2x from either side gives that $x=1$. Substituting this back into our equation gives that $y=\ln(3)$ or approximately 1.099.
