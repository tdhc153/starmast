...
Another type of equation will be a quadratic equation cleverly disguised as a trigonometric equation. This is almost the opposite method from the one just above it. For this, you will want to treat the trigonometric function as the variable and solve the quadratic normally, before using the method above to find the final answer. For example, for $4\sin^{2}(x)+6\sin(x)+7=0$, you would solve the equation $4y^2+6y+7=0$ and then set $y=\sin(x)$ and solve; by this point, y will be a constant. For more information on solving quadratic equations see the Quadratic Equations guide.
