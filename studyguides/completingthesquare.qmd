...
{{< video https://youtu.be/Yl0GZrIdSxY aspect-ratio="16x9">}}
...
<div class="controls">
  b: <input id="bVal" type="number" value="4" step="2">
  c: <input id="cVal" type="number" value="9">

  <br><br>

  <button id="prevBtn" onclick="prevStage()">Previous step</button>
  <button id="nextBtn" onclick="nextStage()">Next step</button>
...
function validate() {
  ...
  if (b < 0) {
    warning.textContent = "b must be non-negative";
    return false;
  }

  if (b % 2 !== 0) {
    warning.textContent = "b must be even";
    return false;
  }
...
function drawInitial() {
  ...
  for (let i = 0; i < c; i++) {
    ...
    t.style.left = (x + (i % 6) * 22) + "px";
    t.style.top = (150 + Math.floor(i / 6) * 22) + "px";
...
function splitStrips() {
  ...
  strips.forEach((s, i) => {
    if (i < n) {
      ...
      s.style.width = "20px";
      s.style.height = size + "px";
      s.style.left = (baseX + size + i * 20) + "px";
      ...
      s.style.top = (baseY + size + (i - n) * 20) + "px";
    }
  });
}
...
function completeSquare() {
  ...
  missing.style.left = (baseX + size) + "px";
  missing.style.top = (baseY + size) + "px";
  missing.style.width = (n * 20) + "px";
  missing.style.height = (n * 20) + "px";
  ...
  tiles.forEach((t, i) => {
    if (i < needed) {
      ...
      t.style.left = (baseX + size + col * 20) + "px";
      t.style.top = (baseY + size + row * 20) + "px";
    } else {
      // 🔥 FIXED GRID (10 per row)
      const col = (i - needed) % 10;
      const row = Math.floor((i - needed) / 10);

      t.style.left = (baseX + size + n * 20 + 60 + col * 20) + "px";
      t.style.top = (baseY + row * 20) + "px";
    }
  });

  if (c < needed) {
    missing.style.background = "rgba(255,0,0,0.3)";
  }
}
...
function updateMath() {
  ...
  result.innerHTML =
    `\\[x^2 + ${b}x + ${c} = (x + ${n})^2 ${sign} ${val}\\]`;

  MathJax.typesetPromise();
}

/* ---------------- EVENTS ---------------- */
bVal.addEventListener("input", drawInitial);
cVal.addEventListener("input", drawInitial);
