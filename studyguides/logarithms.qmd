...
![Figure 1: A graph that shows the natural logarithm $\ln(x)$, along with the $e^x$ curve. Each curve is a reflection of the other in the line $y = x$, making them inverses of each other. Note that $\ln(x)$ only takes positive $x$ values, and is undefined for $x = 0$.](./FiguresPNG/log_curve.png){fig-alt="A graph of the natural logarithm and it's inverse, the exponential function with Euler's number as the base."}
