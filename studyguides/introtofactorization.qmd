...
Factorization is the process of rewriting an expression as a product of simpler expressions. This makes it easier to work with and understand more complicated expressions.

As you have seen in [Guide: Introduction to quadratic equations](introtoquadratics.qmd), quadratic expressions are important in mathematics, and these can be manipulated by using factorization to help you solve them. For example, factorizing the expression $x^2 + 5x + 6$ transforms it into $(x+3)(x+2)$, which makes it easier for you to find the values of $x$ that satisfy the equation $x^2 + 5x + 6 =0$.
...
$3x + 5$ is an expression. It contains the number $3$, multiplied by the variable $x$, and the addition operation.
:::
...
A **factor** is an expression that divides exactly into another expression. 
:::
...
When working with fractions, any non-zero number can be a common factor. For example, $\dfrac{1}{2}$ is a factor of $1$ and $2$. 

**This guide will focus on whole number factorizations.**
...
The **highest common factor (HCF)** is the largest factor by which two or more terms can be divided by, without leaving a remainder. It is the greatest number that divides all the given terms evenly and is divisible by all other common factors of the numbers or expressions involved.
:::
...
The terms are $5x$ and $15$ and the terms have a common factor of $5$.

Factorizing out the $5$ from both terms gives: $$5x+15=5(x)+5(3)$$There are no other common factors of $x$ and $3$, so $5$ is the HCF. So, the final factorized form is: $$5x+15=5(x+3)$$
:::
...
The terms have a common factor of $3$. Factorizing out the $3$ from both terms gives: $$6x^2+9x=3(2x^2)+3(3x)$$ 

Now looking at $2x^2$ and $3x$, both terms also contain an $x$, so you can factorize $x$ out aswell to give: $$6x^2+9x=3x(2x)+3x(3)$$

There are no other common factors between $2x$ and $3$, so the HCF is $3$. So, the final factorized form is: $$6x^2+9x=3x(2x+3)$$
:::
...
Factorizing out $(x-4)$ from both terms gives: $$x(x-4)+3(2x-8)= (x-4)(x)+(3)(2(x-4))$$ 
Expanding $(3)(2(x-4))$ out gives: $$(3)(2(x-4)) = (6)(x-4)$$
So the expression becomes:$$x(x-4)+3(2x-8)= (x-4)(x)+(6)(x-4)$$ 
...
Factorizing quadratic expressions involves breaking down quadratic expressions (of the form $ax^2+bx+ c$, where $a \neq 0$) into a product of two brackets, which can make it easier to solve the quadratic expression. This process is essential in algebra and helps simplify expressions, whilst also being able to solve quadratic equations and find their roots. For more information on this, see [Guide: Introduction to quadratic equations](introtoquadratics.qmd).

There are two main cases when factorizing quadratics that you will learn about. One, where $a = 1$, and another where $a \neq 1$.

## Case 1: $a = 1$

When the quadratic expression has the form $x^2+bx+c$, the aim is to find two numbers $d$ and $e$ that multiply to $c$, and add to $b$, such that $de = c$ and $d+e=b$. 

This works because: $$(x+d)(x+e) = x^2+dx+ex+de$$ 

And when factorizing the $x$ term, you get:$$(x+d)(x+e) = x^2+(d+e)x+de$$ 

Which is the desired form of the quadratic expression, as $b=d+e$ and $c=de$.

However, it is best to see this explained with an example.
...
$$1\cdot 10=10 \quad \text{and} \quad2 \cdot 5=10$$
...
$x^2 -x -2$ is not the original quadratic expression you were asked to factorize, which shows you how if a factor has the incorrect sign, it will not correctly factorize the required quadratic expression.
:::
...
If you let $d \cdot e = a \cdot c = f$, then $\dfrac{d}{a}=\dfrac{c}{e}=f$, so you can substitute this in to give:
$$(ax^2+dx) + (ex+c) = ax(x+f) + e(x + f)$$
This means that both groups will share a common factor $(x+f)$, and the expression can be factorized further to give:
$$(ax^2+dx) + (ex+c) = (ax+e)(x+f)$$
This is the fully factorized form of the quadratic expression $ax^2+bx+c$ when $a \neq 1$.


These are the steps to factorize a quadratic:
...
$$2 \cdot 3=6$$
Now you need to consider all the factors of $6$. 
...
$$2+3=5\quad \textsf{and} \quad 2\cdot 3=6$$
...
You need to find the factors of $-15$ that multiply to give $-15$, and add to give $-2$. These are:
...
::: {.callout-important}
Note that a quadratic $ax^2+bx+c$ can sometimes be factorized in the form: $$(px+q)(rx+s)\quad \textsf{where} \quad r \neq 0$$

However, if you factorize out $r^2$, you get:
...
$$\left( \dfrac{px}{r} + \dfrac{q}{r} \right) \left( x+\dfrac{s}{r} \right) = \left( ax+b \right) \left( x+c \right) 
\quad \textsf{, where} \quad a=\dfrac{p}{r} \textsf{,} \quad b=\dfrac{q}{r} \quad \textsf{and} \quad c=\dfrac{s}{r}$$
:::
...
Now you can solve each one for $x$: $$x=2\quad \text{and} \quad x=3$$ 

Once solved, you can see the solutions of $x^2-5x+6$ are $x=2$ and $x=3$.
:::

Using tools like Desmos, you can verify your solutions by graphing the quadratic equation and identifying the points where it intersects with the $x$-axis. The roots of the quadratic equations are the points of intersection with the $x$-axis.

See [Calculator: Solving quadratic equations] to see this in action!
...
[For more questions on the subject, please go to Questions: Introduction to factorization.](../questions/qs-completingthesquare.qmd)

[For more information on solving quadratic equations, please see Guide: Introduction to quadratic equations.](introtoquadratics.qmd)
