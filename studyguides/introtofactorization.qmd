...
As you have seen in [Guide: Introduction to quadratic equations](introtoquadratics.qmd), quadratic expressions are important in mathematics, and these can be manipulated by using factorization to help you solve them. For example, factorizing the expression $x^2 + 5x + 6$ transforms it into $(x+3)(x+2)$, which makes it easier for you to find the values of $x$ that satisfy the equation.
...
$3$ is a factor of $12$, because $12 \div 3 = 4$. Similarly, $2x$ is a factor of $6x$, because $6x \div 2x = 3$.

:::

::: {.callout-important}

When working with fractions, any non-zero number can be a common factor. For example, $\frac{1}{2}$ is a factor of $1$ and $2$. 
...
$4x \div 2 = 2x \quad  \text {and} \quad 6 \div 2 = 3$.
...
Factorizing quadratic expressions involves rewriting a quadratic expression (in the form $ax^2+bx+ c$, where $a \neq 0$) as a product of expressions. This process is essential in algebra and helps simplify expressions, whilst also being able to solve quadratic equations and find their roots. For more information on this, see [Guide: Introduction to quadratic equations](introtoquadratics.qmd).

There are two main cases when factorizing quadratics that you will learn about. One, where $a = 1$, and another where $a \neq 1$.

## Case 1: $a = 1$

When the quadratic expression has the form $x^2+bx+c$, the aim is to find numbers $d$ and $e$ that multiply to $c$, and add to $b$, such that $de = c$ and $d+e=b$. 

This works because: $$(x+d)(x+e) = x^2+dx+ex+de$$ 

And when factorising the $x$ term, you get:$$(x+d)(x+e) = x^2+(d+e)x+de$$ 

Which is the desired form of the quadratic expression.
...
The factors of $-2$ are $1, -1, 2$, and $-2$. You want to look at the factors and see which pairings multiply to make $-2$ and add to $1$. These are: $$2 \cdot (-1)=-2$$ $$2+(-1)=1$$So $2$ and $-1$ are factors of the quadratic.
...
$x^2 -x -2$ is not the original quadratic expression you were asked to factorize, which shows you how if a factor has the incorrect sign, it will not correctly factorize the quadratic expression you are trying to factorize.

## Case 2: $a \neq 1$
When the quadratic expression has the form $ax^2+bx+c$, it can be factorized as follows: $$ax^2+bx+c=(ax+b)(x+c)$$
The aim is to find two numbers $d$ and $e$ such that:
$$d \cdot e = a \cdot c \quad \text{and} \quad d + e =b$$
This works because:
$$(dx+e)(x+f) = dx^2+ex+dfx+ef$$
And when factorizing the $x$ term you get:$$(dx+e)(x+f) = dx^2+(e+df)x+ef$$
Which is the desired factorized form. 
