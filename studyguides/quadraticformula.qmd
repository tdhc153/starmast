...
*Before reading this guide, it is highly recommended that you read [Guide: Introduction to quadratic equations](introtoquadratics.qmd). In addition, the proof of the quadratic formula relies on the technique of 'completing the square'; see (Guide: Completing the square) for more.*
...
Find the roots of each of the quadratic equations below. If a root is repeated, write it twice. Complex numbers should be written as $a+bi$ without spaces, and where $a$, $b$ are real numbers. If the roots are real, write them in ascending order; if the roots are complex, write the negative $i$ term first. 
...
[For more questions on the subject, please go to Questions: Using the quadratic formula.](qs-quadraticformula.qmd)
