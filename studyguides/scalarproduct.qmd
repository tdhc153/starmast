...
Similarly, in column notation $\mathbf{a}= \begin{pmatrix}a_1\\ a_2 \\a_3\end{pmatrix}$ and $\mathbf{b} =\begin{pmatrix}b_1\\ b_2 \\b_3\end{pmatrix}$, then:
...
Take $\mathbf{a}=\begin{pmatrix}3\\5\\1\end{pmatrix}$ and $\mathbf{b} =\begin{pmatrix}2\\7\\2\end{pmatrix}$, then the scalar product is $$\begin{pmatrix}3\\ 5 \\1\end{pmatrix}\cdot\begin{pmatrix}2\\ 7 \\2\end{pmatrix} = (3)(2) + (5)(7) + (1)(2) = 6+35+2 = 43$$
:::
...
Take $\mathbf{a}=\begin{pmatrix}-1\\7\\1\end{pmatrix}$ and $\mathbf{b} =\begin{pmatrix}-1\\-1\\2\end{pmatrix}$, then the scalar product is $$\begin{pmatrix}-1\\ 7 \\1\end{pmatrix}\cdot\begin{pmatrix}-1\\ -1 \\2\end{pmatrix} = (-1)(-1) + (7)(-1) + (1)(2) = 1-7+2 = -4$$
:::
...
::: {.callout-note appearance="simple"}
## Example 4
Take $\mathbf{a}=\begin{pmatrix}3\\5\\1\end{pmatrix}$ and $\mathbf{b} =\begin{pmatrix}2\\7\\2\end{pmatrix}$, as in Example 1. In that example, it was calculated that $\mathbf{a}\cdot\mathbf{b} = 43$. So you can use the above definition to calculate the smallest angle $\theta$ between the two vectors.
...
Take $\mathbf{a}=\begin{pmatrix}-1\\7\\1\end{pmatrix}$ and $\mathbf{b} =\begin{pmatrix}-1\\-1\\2\end{pmatrix}$, as in Example 2. It was calculated then that the scalar product between them is $-4$. So, the angle between them can be calculated as follows:
...
Take $\mathbf{a}=\begin{pmatrix}3\\5\\1\end{pmatrix}$ and $\mathbf{b} =\begin{pmatrix}2\\7\\2\end{pmatrix}$, as in Example 1. Then you know $\mathbf{a}\cdot\mathbf{b}= 43$. You can check $\mathbf{b}\cdot\mathbf{a}$ to verify the commutative property (1) of the scalar product: $$\mathbf{b}\cdot\mathbf{a}=\begin{pmatrix}2\\7\\2\end{pmatrix}\cdot\begin{pmatrix}3\\5\\1\end{pmatrix}=(2)(3)+(7)(5)+(2)(1)=6+35+2=43$$ So $\mathbf{a}\cdot\mathbf{b}=\mathbf{b}\cdot\mathbf{a}$ as expected.
...
Take $\mathbf{a}=\begin{pmatrix}14\\2\\100\end{pmatrix}$, then $$\mathbf{a}\cdot\mathbf{a}=14^2+2^2+100^2=10200.$$ The magnitude of $\mathbf{a}$ is $$|\mathbf{a}|=\sqrt{14^2+2^2+100^2}=\sqrt{10200}$$ and so, $$|\mathbf{a}|^2=\mathbf{a}\cdot\mathbf{a}$$ as you would expect!
:::
...
::: {.callout-note appearance="simple"}
## Example 9
Let $\mathbf{a}=\begin{pmatrix}2\\1\\z\end{pmatrix}$ and $\mathbf{b}=\begin{pmatrix}5\\6\\1\end{pmatrix}$. By working out the scalar product of $\mathbf{a}$ and $\mathbf{b}$ and setting $\mathbf{a}\cdot\mathbf{b} = 0$, you can find the value of $z$ for which $\mathbf{a}$ and $\mathbf{b}$ are perpendicular. Doing this gives
$$\mathbf{a}\cdot\mathbf{b}=\begin{pmatrix}2\\1\\z\end{pmatrix}\cdot\begin{pmatrix}5\\6\\1\end{pmatrix}=(2)(5)+(1)(6)+(z)(1)=10+6+z=16+z$$
...
1.  Using the algebraic definition, what is the scalar product of the vectors $\mathbf{a}=\begin{pmatrix}1\\2\\-1\end{pmatrix}$ and $\mathbf{b}=\begin{pmatrix}2\\10\\15\end{pmatrix}$?
...
3. Working in degrees, find the angle between the vectors $\mathbf{a}=\begin{pmatrix}7\\-4\\11\end{pmatrix}$ and $\mathbf{b} =\begin{pmatrix}5\\2\\2\end{pmatrix}$.
...
3. Working in degrees, find the angle between the vectors $\mathbf{a}=\begin{pmatrix}7\\-4\\11\end{pmatrix}$ and $\mathbf{b} =\begin{pmatrix}5\\2\\2\end{pmatrix}$. You should give your answer accurate to the nearest whole degree. 
