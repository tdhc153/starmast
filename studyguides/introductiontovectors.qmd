...
```{=html}
<table><tr><td style="vertical-align: middle"><strong>Narration of study guide:</strong>&nbsp;&nbsp;</td><td><audio controls><source src="./Narrations/introductiontovectors.mp3" type="audio/mpeg">Your browser does not support the audio element.</audio></tr></table>
```
...
<!-- To find if the vector in [Example 9](#example-9-link) and $\mathbf{d}=\begin{bmatrix}6\\-8\\10\end{bmatrix}$\ are parallel or not, you check if the corresponding coefficients have the same ratio.  -->
...
::: {.webex-check .webex-box data-topic="ITV1"}
...
2. If $\mathbf{a}=\begin{bmatrix}2\\-2\\-1\end{bmatrix}$, then what is $|\mathbf{a}|$?
