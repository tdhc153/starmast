...
abstract: Quantities expressing both a magnitude and a direction are called vectors. Vectors are used in various disciplines and have a wide range of real-life applications.
categories:
  - Geometry
  - Vectors
  - Linear mathematics
image: FiguresPNG/introductiontovectors-image.png
---
...
<!-- A **vector** is a quantity that possesses both *magnitude* (size, or length) and *direction*, and so these can be used to model the travel to your friendly gathering above. Quantities that represent magnitude only are called **scalars** -->
...
<!-- Examples of scalar quantities in physics include mass, time, distance, volume, temperature, and so on; examples of vector quantities in physics are displacement, velocity, momentum, among others. -->
...
<!-- This guide introduces the concept of a vector including what they are in both geometry and algebra and the different ways in which you can write them such as column notation and component. This guide also introduces the magnitude (length) of a vector and the idea of a unit vector." -->
...
![A vector $\mathbf{v}$](./FiguresPNG/introtovectors-fig0.png){fig-alt="A picture of an arrow, with label v in bold." #fig-1}
...
![Many copies of the vector $\mathbf{v}$](./FiguresPNG/introtovectors-fig01.png){fig-alt="Five copies of the vector $\mathbf{v}$, each with a label of a bold v." #fig-2}
...
This study guide will use the bold notation ($\mathbf{a}$). However, vectors can also be represented by using the arrow notation $\vec{a}$ or (as previously mentioned) underlining them $\underline{a}$.
...
![Graphical representation of the vector $\overrightarrow{AB}$](./FiguresPNG/introtovectors-fig1-1.png){fig-alt="Vector $\overrightarrow{AB}$, with arbitrary length, looks like an arrow pointing from $A$ to $B$, where, $A$ is the tail and $B$ is the head." #fig-3}
...
![Graphical representation of the vector $\overrightarrow{BA}$](./FiguresPNG/introtovectors-fig2-2.png){fig-alt="Vector $\overrightarrow{BA}$, with arbitrary length, looks like an arrow pointing from $B$ to $A$, where, $B$ is the tail and $A$ is the head." #fig-4}
...
![The position vector for the point $P$ with respect to the origin](./FiguresPNG/introtovectors-fig4-4.png){fig-alt="Position Vector $\overrightarrow{OP}$ with its tail at the origin and head at an arbitrary point $P$." #fig-5}
...
For more on scalar multiplication on vectors, please see [Guide: Vector addition and scalar multiplication](addandsm.qmd). 
...
![$\mathbf{i},\mathbf{j},\mathbf{k}$ unit vectors along their respective axis.](./FiguresPNG/introtovectors-fig5-5.png){fig-alt="$\mathbf{i},\mathbf{j},\mathbf{k}$ unit vectors along $x,y,z$ axis." #fig-6}
...
You are given the vector $\mathbf{c}=3\mathbf{i}-4\mathbf{j}+5\mathbf{k}$. To represent this vector in column notation you can write the components of the vector as specified above. So, $$\mathbf{c}=3\mathbf{i}-4\mathbf{j}+5\mathbf{k}=\begin{pmatrix}3\\-4\\5\end{pmatrix}.$$
...
To express the vector $\displaystyle \mathbf{d}=\begin{pmatrix}-7\\0\\4\end{pmatrix}$\ in terms of $\mathbf{i},\mathbf{j},\mathbf{k}$, you can reverse the process given above to write $$\mathbf{d}=\begin{pmatrix}-7\\0\\4\end{pmatrix}=(-7)\mathbf{i}+0\mathbf{j}+4\mathbf{k} = -7\mathbf{i} + 4\mathbf{k}.$$
...
How do you express the zero vector $\mathbf{0}$ in both of these vector notations? Here, the values of each of the components of the zero vector are $0$, so $$\mathbf{0} = 0\mathbf{i} + 0\mathbf{j} + 0\mathbf{k} = \begin{pmatrix}0\\0\\0\end{pmatrix}.$$

How do you express the negative of a vector $\mathbf{a} = x\mathbf{i} + y\mathbf{j} + z\mathbf{k} = \begin{pmatrix}x\\y\\z\end{pmatrix}$? The negative of a vector is its reverse; so all of the components are multiplied by $-1$. Therefore $$-\mathbf{a} = (-x)\mathbf{i} + (-y)\mathbf{j} + (-z)\mathbf{k} = \begin{pmatrix}-x\\-y\\-z\end{pmatrix}.$$
...
Let $\displaystyle\mathbf{a} = x\mathbf{i}+y\mathbf{j}+z\mathbf{k} = \begin{pmatrix}x\\y\\z\end{pmatrix}$ be a vector. The **magnitude** of $\mathbf{a}$ is given by $$|\mathbf{a}| = \sqrt{x^2 + y^2 + z^2}$$
...
<!-- dividing You can use the magnitude of your vector $\mathbf{a}$ to scale the vector. By dividing by the magnitude ( -->

<!-- Any vector $\mathbf{a}=x\mathbf{i}+y\mathbf{j}+z\mathbf{k}$\ can have its unit vector expressed as  -->

<!-- $\hat{\mathbf{a}}=\frac{\mathbf{a}}{|\mathbf{a}|}=\frac{x\mathbf{i}+y\mathbf{j}+z\mathbf{k}}{|\mathbf{a}|}=\frac{x\mathbf{i}+y\mathbf{j}+z\mathbf{k}}{\sqrt{x^2+y^2+z^2}}$\ -->

<!-- So, $\hat{\mathbf{a}}=\frac{x\mathbf{i}+y\mathbf{j}+z\mathbf{k}}{\sqrt{x^2+y^2+z^2}}$\ -->
...
1. State whether these statements are true or false.
    ...
    (a) The magnitude of a vector is always a non-negative value.
        Answer: `r torf(TRUE)`.

    (b) The zero vector has a magnitude of one.
        Answer: `r torf(FALSE)`.
        
    (c) If $\mathbf{v}$ is a non-zero vector, then there exists a unit vector in the direction of $\mathbf{v}$. Answer: `r torf(TRUE)`.
    

2. If $\mathbf{a}=\begin{pmatrix}2\\-2\\-1\end{pmatrix}$, then what is $|\mathbf{a}|$?

    Answer: $|\mathbf{a}|$ is equal to `r fitb(3)`.

3. Find the unit vector in the direction of $\mathbf{b}=-3\mathbf{i}+4\mathbf{k}$. (Give your answers using decimals.)

    Answer: $\hat{\mathbf{b}}$ is equal to `r fitb(-0.6)`$\mathbf{i}+$`r fitb(0.8)`$\mathbf{k}$.
...
1. State whether these statements are true or false.
    ...
    (c) If $\mathbf{v}$ is a non-zero vector, then there exists a unit vector in the direction of $\mathbf{v}$.

2. If $\mathbf{a}=\begin{pmatrix}2\\-2\\-1\end{pmatrix}$\, then what is $|\mathbf{a}|$?

3. Find the unit vector in the direction $\mathbf{b}=-3\mathbf{i}+4\mathbf{k}$.
...
[For more questions on this topic, please go to Questions: Introduction to vectors.](../questions/qs-introductiontovectors.qmd)

To explore more about vectors, included how they are added and scalar multiplied, please see either [Guide: Vector addition and scalar multiplication](addandsm.qmd).
