...
You can use two different but equivalent methods to decide whether or not to reject your hypothesis to the stated significance level. These methods depend on **the probability distribution being used**; see [Guide: PMFs, PDFs, CDFs](./studyguides/pmfspdfscdfs.qmd) and [Overview: Probability distributions] for more.
...
If your probability distribution is **symmetric** (like the normal distribution), then $z_1 = -z_2$ in a two-tailed test. If it is not symmetric (like the $\chi^2$ distribution).
...
- If you have an upper-tailed test $$H_1: \mu>\mu_0$$ (where $\mu_0$ is the value you are testing your sample data against), then the $p$-value is the area to the **right** of the test statistic under the probability distribution curve. See @fig-1 for a picture.
...
This has a different impact based on whether you are performing a one-tailed or a two-tailed test. As shown in the graph below, for a one-tailed test, that whole $5\%$ rejection region will be at one end of the results. This is because you are only testing whether the characteristic is greater than or equal to our comparative characteristic. In the case of a two-tailed test, that $5\%$ will be split across either end of the distribution to reject the $2.5\%$ at both far left and far right of the curve. This is because you are testing whether the test characteristic is both greater than or less than the comparative characteristic. 
...
Your null hypothesis is that the average weight of the bars is $\mu_0 = 5$ grams and your alternative hypothesis is the average weight of bars is less than $\mu_0 = 5$ grams. 
...
In this case you have one sample of $30$ bars and you know the population standard deviation. You are also testing the alternative hypothesis that $\mu < 5$. This implies that you would want to use a **lower one-tailed $Z$-test**. This means calculating a test statistic according to a $Z$-test, which is outlined here.

First, you would need to calculate the standard error from the mean is calculated to be $$SE = \frac{\sigma}{\sqrt{n}} = \frac{1.5}{\sqrt{30}}$$

Next, the standard score is given by 

$$Z = \frac{\bar{x} - \mu_0}{SE} = \frac{4.5 - 5}{\frac{1.5}{\sqrt{30}}} \approx -1.826$$
(See [Guide: Introduction to $Z$-testing] for more.)

**Find the critical value or $p$-value**

Since this is a lower one-tailed test, you need to find the critical value for $\alpha=0.05$ from the $Z-$table. The critical $Z-$value for a significance level of $0.05$ in a lower one-tailed test is $-1.645$.

Alternatively, you can find the $p-$value associated with the test statistic $Z=−1.826$. From the $Z-$table, you will find that a $Z-$score of $-1.826$ corresponds to a $P-$value of approximately $0.033925$.

**Conclusion**

If your $Z-$score is less than the critical value $-1.645$ or $P≤\alpha$, you can reject the null hypothesis $H_0$.If neither condition is met, you fail to reject the null hypothesis.

In this case, the calculated $Z=-1.826$, which is less than the critical value of $-1.645$. Similarly, $p=0.033925$, which is smaller than $\alpha=0.05$.

Since $Z=−1.826$ is less than $-1.645$ (equivalently, the $p-$value is smaller than $0.05$), you can reject the null hypothesis $H_0$. There is enough evidence to conclude that the average weight of the Boole Bars is less than $5$ grams.
