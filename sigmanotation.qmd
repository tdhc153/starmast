...
Sometimes, you'll want to multiply two sums together. This can be succinctly using something called *double sums*.

::: {.callout-note appearance="simple"}
## Double sums
  Let $a_{k},a_{k+1},\ldots,a_n$ and $b_t,b_{t+1},\ldots,b_m$ be two sequences of numbers (where $k,n,t$, and $m$ are integers with $k \leq n$ and $t \leq m$). Then the double sum $sum_{i=k}^n\sum_{j=t}^m a_i b_j$ is defined as  $$\sum_{i=k}^n\sum_{j=t}^m a_i b_j = a_kb_t + a_kb_{t+1} + \ldots + a_kb_m + a_{k+1}b_t + a_{k+1}b_{t+1} + \ldots + a_{k+1}b_m + \ldots + a_nb_m.$$  
:::
