...
abstract: A selection of questions for the study guide on introduction to partial differentiation.
draft: true
---
...
Find the first-order partial derivatives of each function $f$.
...
1.4. $\displaystyle \quad f(x,y) = \mathrm{e}^{xy} + 2x^2y^3$
...
1.11. $\displaystyle \quad f(x,y) = (x^2+2xy) \ln(xy)$

1.12. $\displaystyle \quad f(x,y) = \sqrt{x} \tan(y) + 5x \ln(x^2) \cos(2y)$
...
Show that each function is harmonic by calculating its second-order partial derivatives and checking that their sum is zero.
...
2.5. $\displaystyle \quad f(x,y) = \mathrm{e}^x \sin(y)$

## Q3 {.unnumbered}

For each function $f(x,y)$, calculate the mixed partial derivatives and show that they satisfy the equation
